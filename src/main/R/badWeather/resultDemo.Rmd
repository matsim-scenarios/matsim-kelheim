...
result_data <- result_data %>% 
  ...
  distinct() %>%
  mutate(date_simplified = as.integer(date) - as.integer(min(result_data$date)))
...
#   dplyr::select(-weather_impact,-description ,-date,-season,-noRequests,-avgEuclidianDistance_m,-avgTravelTime_s,-wday, -tsun, -isHoliday)
# print(cor(test_cor))

best_pred <- result_data  %>% ungroup() %>%
  dplyr::select(-noRides,-description ,-date, -date_simplified,-season,-noRequests,-avgEuclidianDistance_m,-avgTravelTime_s,-wday) %>%
...
colors <- c("observed" = "blue","predicted" = "red")
model <- omega_model
...
#   geom_point(aes(x = date,y = noRides,color = "observed"))+
...
ggplot(test_data %>% filter(year(date)>=2020)) +
  ...
  scale_x_date(date_breaks = "4 month", date_labels = "%d/%b/%y") +
  scale_color_manual(values = colors) +
  ggtitle("First Linear regression model")
...
omega_date_model <- lm(noRides ~ stringency+wspd+wpgt+wdir+tdiff+tmax+tavg+tmin+pres+date_simplified,data = data)
...
cor_tmin_tavg <- cor(test_data$tmin, test_data$tavg)
print(paste("correlation of tmin and tavg:",cor_tmin_tavg))
...
For reasons of dimensioning the given date variable is transformed into an integer, which represents the number of days, which have passed since the KEXI service has started. The inclusion of the subsequent date parameter ("date_simplified") improves the model accuracy to 0.7845. This goes in line with a decrease of the residual standard error by 8.62 to 16.09. The distribution of the predicted model values as well as the residuals show a typical form. To further check the model's correctness, a histogram of the residuals is plotted.
...
reduced_1_model <- lm(noRides ~ stringency+tavg+date_simplified, data = data)
...
reduced_2_model <- lm(noRides ~ stringency+tmin+date_simplified, data = data)
...
A last correlation check of the left variables (number of KEXI rides, tavg and date_simplified) displays that the two independent variables tavg and date_simplified show a correlation of around 0.13 with each other. This fact should be taken into account when building the final linear regression model. A new parameter dateDependentTemperature = tavg * date_simplified is created and added as a further independent variable of the final model. The resulting model presents the correlation parameter to be non-significant. Further, an exclusion of the newly created variable from the linear regression model does not decrease the model's accuracy, so the correlation variable is removed from the analysis.
...
cor_check <- data %>%
  dplyr::select(noRides,tavg,date_simplified)
print(cor(cor_check))

data <- data %>%
  mutate(dateDependentTemperature = tavg * date_simplified)
reduced_3_model <- lm(noRides ~ tavg+date_simplified, data = data)
...
ggplot(test_data %>% filter(year(date)>=2020)) +
  ...
  scale_x_date(date_breaks = "4 month", date_labels = "%d/%b/%y") +
  scale_color_manual(values = colors) +
  ggtitle("Linear regression model with independent variables tavg and date_simplified")

ggplot(test_data %>% filter(year(date)>=2020)) +
geom_point(aes(x = pred,y = noRides)) +
geom_abline(aes(intercept = 0, slope = 1), size = 1.5, color = "forestgreen") +
theme_minimal() +
...
ggplot(test_data %>% filter(year(date)>=2020))+
      ...
      axis.ticks.y = element_line(),
      axis.ticks.length = unit(5, "pt"), legend.position = "none") +
            ggtitle("Residuals over time for linear regression model with independent variables tavg + date_simplified")
...
ggplot(test_data %>% filter(year(date)>=2020), aes(x = pred,y = resid))+
  ...
  geom_smooth(method ="loess", se = FALSE, color = "#666666", size = 1.5) +
      ...
      axis.ticks.y = element_line(),
      axis.ticks.length = unit(5, "pt"), legend.position = "none") +
            ggtitle("Residuals over predicted values for linear regression model with independent variables tavg + date_simplified")

```

The final linear regression model projects the daily number of KEXI rides as the dependent variable with the variables tavg and date_simplified as independent variables. With p-values of < 2e-16 both independent variables are of very high significance for explaining the dependent variable. The final adjusted R-squared value is 0.7847, which means that 78% of variance in the dependent variable (daily number of KEXI rides) can be explained by the independent variables.
...
barplot <- ggplot(test_data, aes(x = resid ))+
  ...
  ggtitle("Final residuals distributions with independent variables tavg + date_simplified")
