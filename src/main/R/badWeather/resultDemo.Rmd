...
# Stringency
json <- fromJSON(file = "C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/2022-12-31.json")
json <- unlist(json)
#Mobility
demand <- read_delim("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/allDemandByDate.csv")

requests <- read_delim("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/allRequestsByDate.csv")
...
holidays2023 <- read_csv2("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/Holidays2023.csv") %>% dplyr::select(1,2,3)
...
#%>% filter(noRides != 0) #%>% filter(date <"2021-07-01")
...
wday_plot <- ggplot(plot_data)+
  geom_boxplot(aes(x = wday_char,y = noRides,col=wday_char)) +
  xlab("Weekday") + 
...
omega_model <- lm(noRides ~ stringency+wspd+wpgt+wdir+snow+tmax+tavg+tmin+pres,data = data)
...
# colors <- c("Sun" = "darkorchid4","predicted" = "red", "Mon" = "darkblue", "Tue" = "deepskyblue4", "Wed" = "deepskyblue2", "Thu" = "cadetblue4", "Fri" = "chartreuse4", "Sat" = "darkgoldenrod2", "residuals" = "gray50" )
...
ggplot(test_data %>% filter(year(date)>=2020)) +
  ...
  geom_line(mapping=aes(x = date,y = pred),col="red", size = 1.2)+
  theme_minimal() +
  xlab("Date") +
...
omega_date_model <- lm(noRides ~ stringency+wspd+wpgt+wdir+snow+tmax+tavg+tmin+pres+trend,data = data)
...
ggplotly(ggplot(test_data %>% filter(year(date)>=2020)) +
  geom_point(aes(x = date,y = noRides,col=wday_char))+
  geom_point(aes(x = date,y = pred),col="red")+
  #scale_color_manual(values = colors)+
...
reduced_1_model <- lm(noRides ~ stringency+pres+trend, data = data)
...
reduced_2_model <- lm(noRides ~ pres+trend, data = data)
...
cor_check <- data %>%
  dplyr::select(noRides,tavg,trend)
print(cor(cor_check))

data <- data %>%
  mutate(dateDependentTemperature = tavg * trend)
reduced_3_model <- lm(noRides ~ pres+trend, data = data)
...
ggplot(test_data %>% filter(year(date)>=2020)) +
  geom_point(aes(x = date,y = noRides,col=wday_char))+
  geom_line(aes(x = date,y = pred),col = "red", size = 1.2)+
  theme_minimal() +
  xlab("Date") +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  theme(axis.ticks.x = element_line(), 
  ...
  #scale_color_manual(values = colors) +
  ggtitle("Linear regression model with independent variables pres and trend")
...
ggplot(test_data %>% filter(year(date)>=2020))+
      ...
      axis.ticks.y = element_line(),
      axis.ticks.length = unit(5, "pt"), legend.position = "none") +
            ggtitle("Residuals over time for linear regression model with independent variables pres + trend")
...
ggplot(test_data %>% filter(year(date)>=2020), aes(x = pred,y = resid))+
  ...
  geom_smooth(method ="loess", se = FALSE, color = "#666666", size = 1.5) +
      ...
      axis.ticks.y = element_line(),
      axis.ticks.length = unit(5, "pt"), legend.position = "none") +
            ggtitle("Residuals over predicted values for linear regression model with independent variables pres + trend")
...
barplot <- ggplot(test_data, aes(x = resid ))+
  ...
  ggtitle("Final residuals distributions with independent variables pres + trend")
