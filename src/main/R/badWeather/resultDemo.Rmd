---
title: "Linear regression model on Kelheim weather data"
...
# Weatherstack data
weatherstack_kelheim <- read_delim("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/Kelheim_weather_since_july_2008.csv",delim = ",")

# Stringency
json <- fromJSON(file = "C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/2022-11-10.json")
json <- unlist(json)

#Mobility
demand <- read_delim("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/allDemandByDate2022.csv")

#Expanding the data
demand2022 <- read_delim("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/VIA_Rides_202201_202210.csv")

#Holidays
holidays2020 <- read_csv2("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/Holidays2020.csv") %>% dplyr::select(1,2,3)
holidays2021 <- read_csv2("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/Holidays2021.csv") %>% dplyr::select(1,2,3)
holidays2022 <- read_csv2("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/Holidays2022.csv") %>% dplyr::select(1,2,3)
...
holiday_plot <- ggplot(result_data)+
...
The correlation analysis shows that only the covid19 policy stringency has a strong influence on the daily number of KEXI rides (-0.56). Weather parameters do not seem to have a high impact, which might be due to the strong effects of the pandemic. Therefore weather parameters with a correlation value of bigger than |0.05| (TODO check if this makes sense!-sm0223) are taken into account. This includes some parameters in relation to wind (wspd, wpgt, wdir), temperature (tdiff, tmax, tavg, tmin) and air (pres).
...
omega_date_model <- lm(noRides ~ stringency+wspd+wpgt+wdir+tdiff+tmax+tavg+tmin+pres+date,data = data)
...
The inclusion of a date parameter the model accuracy to 0.7876, which is a rather high value in weather statistics (TODO ask Sasha where he got that -sm0223) This goes in line with a decrease of the residual standard error by 8.75 to 16.01. The distribution of the predicted model values as well as the residuals show a typical form. To further check the model's correctness, a histogram of the residuals is plotted.
...
reduced_1_model <- lm(noRides ~ stringency+tavg+date, data = data)
...
reduced_2_model <- lm(noRides ~ stringency+tmin+date, data = data)
...
<!-- As we can see removing snow and separation of weather_impact and description, increased model quality with reducing residual standard error. Anova(analysis of variance) test between these two models also doesn't show significant difference `r anova(reduced_2_model,reduced_1_model)[["Pr(>F)"]][2]` because p-value is much greater than 0.05. -->

<!-- After few iterations we get to a model that contains only strictness of the policy restrictions, temperature and data. Represented model explains also explains most of the observations with relatively low residual standard error, but it is minimal for excluding, because anova significance test difference between models without any of remaining predictors has p-value<0.05. So we get to our final model, that explains number of KeXi Rides using temperature, covid strictness level and date(positive growing trend).  -->

<!-- Critical point there, is that using season variable instead of temperature variable doesn't decrease model quality, even slightly increases. As a conclusion temperature plays a role of a season in model by fitting data, because average temperature correlates with season. But filtering a data to a specific season(e.g. summer) with model fit(using temperature), returns a R squared result of 0.809, that is relatively high for an weather analysis. Anova test of model with excluded temperature variable and model with temperature returns a value of 2.14*e^-10, so difference of model with weather and without have significant difference. -->
...
reduced_3_model <- lm(noRides ~ tavg+date, data = data)
...
ggplotly(ggplot(test_data %>% filter(year(date)>=2020)) +
  ...
  geom_point(aes(x = date,y = pred,color = "predicted"))+
  scale_color_manual(values = colors)+
  ggtitle("Linear regression model with independent variables tavg + date"))
```
...
ggplotly(ggplot(test_data %>% filter(year(date)>=2020))+
            ...
            geom_ref_line(h = 0)+
            scale_color_manual(values = colors)+
            ggtitle("Residuals for linear regression model with independent variables tavg + date"))

```

The final linear regression model projects the daily number of KEXI rides as the dependent variable with the variables tavg and date as independent variables. With p-values of < 2e-16 both independent variables are of very high significance for explaining the dependent variable. The final adjusted R-squared value is 0.7872, which means that the calculated model is highly accurate.
...
As the resulting linear regression model has been determined, in a final step the residual distribution has to be checked.

```{r residual destributions 3}

barplot <- ggplot(test_data, aes(x = resid ))+
  geom_histogram(aes(y = stat(density)),colour="black", fill="white", binwidth=9)+
  ggtitle("Final residuals distributions with independent variables tavg + date")

ggplotly(barplot)

```

As before, the residual distribution shows a bell-shape, which assures the model's quality. (@Sidney once again, why is that?) The 2 outliers to the left can again be explained by the operator change in June / July 2021.
...
<!-- So our residuals are normally distributed, that is also verified by shapiro wilk normality test because value of test `r shapiro.test(test_data$resid)$p.value` is much greater than 0.05. -->

TODO: do we even need the qq and pp plot as we already have proof for the normal distribution of our residuals? 
...
A first linear regression model showed a continuous growing trend of daily rides, which led to the conclusion to also integrate a date variable. The inclusion of date improved the model's quality dramatically. In a next step all the non-significant independent weather related variables were excluded. The model's accuracy stayed the same after that step. In a last step covid policy stringency also was excluded from the model because it was non-significant, too. The final linear regression model explains the daily number of KEXI rides (dependent variable) by using variables for average temperature (tavg) and date. Most importantly, the F-statistic for the calculated regression has a p-value of < 2.2e-16. In general, if the p-value is lower than 0.05, the null hypothesis can be revoked, which means that the performed linear regression provides a significant contribution to the change of the daily number of KEXI rides. The model's accuracy is 0.7872 with a residual standard error of 16.03. A scatter plot of actual number of rides vs. predicted number of rides depicts a linear form for the predicted values, which means that a linear regression is the adequate form of analysis. The residuals are distributed normally.
