...
#Mobility
demand = read_delim("data/allDemandByDate2022.csv")
...
Correlation coefficients(Pearson coefficient) between two variables we calculate using cor() function, later anova oneway test will be used for testing to check categorical variable dependency. 
...
As a result highest absolute score in correlation have wspd(average windspeed), wpgt(peak wind gust), tdiff(seasonal temperature difference), pressure(air pressure in hPa), stringency(strictness of covid19 policy). But for the specific season different variables play different role. Let's examine data subset filtered by season. 

```{r, warning = FALSE}
summer_pred = result_data %>% filter(season == "summer")
best_pred <- summer_pred  %>% ungroup() %>%
  dplyr::select(-noRides,-description ,-date,-season,-noRequests,-avgEuclidianDistance_m,-avgTravelTime_s,-wday) %>%
  map_dbl(cor,y = summer_pred$noRides) %>%
  #map_dbl(abs) %>%
  sort(decreasing = TRUE) 
print("summer predictors")
print(best_pred)
```
By comparison with whole dataset we can see that in summer windspeed has less correlation coefficient, and wind directions have different signs. Main sifnificant variable during summer is temperature
```{r, warning = FALSE}
autumn_pred = result_data %>% filter(season == "autumn")
best_pred <- autumn_pred  %>% ungroup() %>%
  dplyr::select(-noRides,-description ,-date,-season,-noRequests,-avgEuclidianDistance_m,-avgTravelTime_s,-wday) %>%
  map_dbl(cor,y = autumn_pred$noRides) %>%
  #map_dbl(abs) %>%
  sort(decreasing = TRUE) 
...
print(best_pred)
```
In autumn pressure, windspeed, snow and temperature have the highest correlation to number of rides.
```{r, warning = FALSE}
spring_pred = result_data %>% filter(season == "spring")
best_pred <- spring_pred  %>% ungroup() %>%
  dplyr::select(-noRides,-description ,-date,-season,-noRequests,-avgEuclidianDistance_m,-avgTravelTime_s,-wday) %>%
  map_dbl(cor,y = spring_pred$noRides) %>%
  #map_dbl(abs) %>%
  sort(decreasing = TRUE) 
print("spring predictors")
print(best_pred)
```
Compared to other seasons in spring precipitation have more impact on number of rides, as well as snow and temperature. 
```{r, warning = FALSE}
winter_pred = result_data %>% filter(season == "winter")
best_pred <- winter_pred  %>% ungroup() %>%
  dplyr::select(-noRides,-description ,-date,-season,-noRequests,-avgEuclidianDistance_m,-avgTravelTime_s,-wday) %>%
  map_dbl(cor,y = winter_pred$noRides) %>%
  #map_dbl(abs) %>%
  sort(decreasing = TRUE) 
...
```
In winter windspeed, pressure and temperature have weak correlation with number of rides.
...
Predicting potential in our model mostly based on growing trend of number of rides related to service start(linear regression model using only date have r squared of >0.7). In future when population get to know service and user quantity will be stabilized our model will provide other results.

```{r model using only a date,out.width="100%"}
#data = data %>% filter(season == "summer")
reduced_3_model = lm(noRides ~ date,data = data)
