...
temp = result_data %>% filter(season == "summer")
...
As a result highest absolute score in correlation have wspd(average windspeed), wpgt(peak wind gust), tdiff(seasonal temperature difference), pressure(air pressure in hPa), stringency(strictness of covid19 policy), weather impact(highest hours of same description a day).
...
reduced_1_model = lm(noRides ~ tavg+pres+stringency+snow+date+wspd,data = data)
...
reduced_2_model = lm(noRides ~ tavg+pres+stringency+date,data = data)
...
reduced_3_model = lm(noRides ~ date+tavg,data = data)
