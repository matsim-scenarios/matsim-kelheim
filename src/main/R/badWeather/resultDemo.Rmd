...
#demand <- read_delim("C:/Users/Simon/Documents/shared-svn/projects/KelRide/data/badWeather/data/allDemandByDate2022_OLD_WRONG_DATA_HANDLING.csv")
...
#Append holidays
result_data <- result_data %>% left_join(df_holidays, by = "date") %>% replace_na(list(isHoliday = FALSE,snow = 0)) %>% mutate(weekday = wday(date))
#%>% filter(noRides != 0) #%>% filter(date <"2021-07-01")

sundays <- result_data %>% 
  filter(wday == 7)

meanRidesSunday <- mean(sundays$noRides)
medianRidesSundy <- median(sundays$noRides)
...
best_pred <- result_data  %>% ungroup() %>%
  dplyr::select(-noRides,-description ,-date, -trend,-season,-wday) %>%
...
omega_date_model <- lm(noRides ~ stringency+wspd+wpgt+wdir+tdiff+tmax+tavg+tmin+pres+trend,data = data)
...
reduced_1_model <- lm(noRides ~ stringency+tavg+trend, data = data)
...
reduced_2_model <- lm(noRides ~ stringency+tmin+trend, data = data)
...
reduced_3_model <- lm(noRides ~ tavg+trend, data = data)
...
ggplot(test_data %>% filter(year(date)>=2020)) +
  ...
  scale_x_date(date_breaks = "4 month", date_labels = "%d/%b/%y") +
  scale_color_manual(values = colors) +
  ggtitle("Linear regression model with independent variables tavg and trend")
...
ggplot(test_data %>% filter(year(date)>=2020))+
      ...
      axis.ticks.y = element_line(),
      axis.ticks.length = unit(5, "pt"), legend.position = "none") +
            ggtitle("Residuals over time for linear regression model with independent variables tavg + trend")
...
ggplot(test_data %>% filter(year(date)>=2020), aes(x = pred,y = resid))+
  ...
  geom_smooth(method ="loess", se = FALSE, color = "#666666", size = 1.5) +
      ...
      axis.ticks.y = element_line(),
      axis.ticks.length = unit(5, "pt"), legend.position = "none") +
            ggtitle("Residuals over predicted values for linear regression model with independent variables tavg + trend")
...
barplot <- ggplot(test_data, aes(x = resid ))+
  ...
  ggtitle("Final residuals distributions with independent variables tavg + trend")
