...
From the p-value we can see that null hypothesis can only be rejected for description (p.value higher than 0.05), for the description and season we can't make such conclusion. But one thing that should be also tested is dependency between seasons and temperature - `r oneway.test(result_data$tavg~result_data$season)$p.value`. As we can see temperature dependent from season, so in our future model one of the variable should be dropped.
...
omega_model = lm(notAtHomeChange ~ tavg+pres+stringency+snow+weather_impact*description,data = data)
...
omega_date_model = lm(notAtHomeChange ~ tavg+pres+stringency+snow+weather_impact*description+date+season,data = data)
...
reduced_1_model = lm(notAtHomeChange ~ tavg+pres+stringency+snow+weather_impact*description+date,data = data)
...
reduced_2_model = lm(notAtHomeChange ~ tavg+pres+stringency+weather_impact+description+date,data = data)
...
reduced_3_model = lm(notAtHomeChange ~ season+tavg+stringency+date,data = data)
