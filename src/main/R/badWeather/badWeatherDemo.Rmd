...
train_data = description_impact %>% filter(year(date)<=2022)
```
...
first_model = lm(not_at_home_change ~ ns(tavg,1)+date+stringency+description,
                 data = train_data)
...
test_data = description_impact %>% add_predictions(model = first_model) %>% add_residuals(model = first_model)
...
ggplotly(ggplot(test_data) +
  ...
  geom_smooth(aes(x = date,y = not_at_home_change),method = "lm",formula = y~x)+
