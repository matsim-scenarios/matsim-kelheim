...
#Append holidays
result_data = result_data %>% left_join(df_holidays, by = "date") %>% replace_na(list(isHoliday = FALSE))
...
## **Build a model**
```{r}
demand_model = lm(noRides ~ tavg+pres+stringency+wday+date+description+isHoliday,data = result_data)
