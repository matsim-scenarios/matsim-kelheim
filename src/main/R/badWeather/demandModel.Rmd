...
#Append holidays
result_data = result_data %>% left_join(df_holidays, by = "date") %>% replace_na(list(isHoliday = FALSE)) #%>% filter(date <"2021-07-01")
...
demand_model = lm(noRides ~ tavg*season+pres+stringency+wday+date+isHoliday+description+snow+prcp,data = result_data)
