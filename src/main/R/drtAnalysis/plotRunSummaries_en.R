...
if(FALSE){
  ...
  plotByConfiguration("Pax per veh-h", "fixed")
  save("singleConfig-paxPerVehHou-enr")
  
  plotByConfiguration("Total pax distance [km]", "fixed")
...
av5_30_avgAvgWait <- av5 %>% 
  filter(parameter == "Avg. wait time") %>% 
  mutate(mean = as.numeric(mean))
av5_30_avgAvgWait <- mean(av5_30_avgAvgWait$mean)
