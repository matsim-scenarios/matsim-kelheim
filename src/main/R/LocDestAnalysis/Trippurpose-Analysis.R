...
p1<-ggplot(counts, aes(x=main_mode, y=Anzahl, fill=end_activity_type))+
...
plotFile <-paste(filePath,"/try.pdf",sep="")
paste("printing plot to ", plotFile)
pdf(plotFile, width = 1200, height = 800)
p1
dev.off()
if(interactiveMode){
  ggplotly(p1)
