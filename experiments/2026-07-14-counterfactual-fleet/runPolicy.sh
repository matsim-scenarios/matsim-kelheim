...
# --export): DEMAND=base|counterfactual, FLEET_SIZE, PREBOOKING=off|on, SEED.
set -euo pipefail
set -x
...
EXPERIMENT_ID="2026-07-14-counterfactual-fleet"
INPUT_DIR="input/v3.1/expanded-service-area"
...
case "$PREBOOKING" in
	off|on) ;;
	*) echo "PREBOOKING must be off or on, got: $PREBOOKING" >&2; exit 2 ;;
esac
...
SCENARIO_ID="${SCENARIO_ID:-demand-${DEMAND}__prebooking-${PREBOOKING}__fleet-${FLEET_SIZE}__seed-${SEED}}"
OUTPUT_DIR="output/${EXPERIMENT_ID}/${SCENARIO_ID}"

for required_file in "$CONFIG" "$FLEET_START_LINK_WEIGHTS" "$EXPANDED_DRT_STOPS" "$STUDY_AREA_SHP"; do
...
if (( ${#jars[@]} != 1 )); then
	echo "Expected exactly one matsim-kelheim-2024.2*.jar in $SCRIPT_DIR, found ${#jars[@]}." >&2
	exit 2
fi
...
if [[ "$PREBOOKING" == on ]]; then
	command+=(--prebooking --prebooking-probability=1.0 --prebooking-submission-slack=1800)
fi
